    ...
    <s-stack gap="base">
      ...
      {state.status === 'hidden' && (
        <s-banner title="SuperApp" tone="info">
          <s-text>
            No action configured for orders. Publish an admin.action module targeting
            admin.order-details.action.render from the SuperApp.
          </s-text>
        </s-banner>
      )}
      {state.status === 'ready' && (
...
export default async function extension() {
  render(<OrderDetailsAction />, document.body);
}
