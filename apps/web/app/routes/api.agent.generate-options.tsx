 ...
 *
 * Returns 3 recipe options. Use POST /api/agent/modules with a selected spec to save.
 * This splits the AI generation step from the persistence step so agents can:
 * - Generate → inspect options → decide → save (or discard)
 * - Generate → present to user → save selected option
 */
export async function action({ request }: { request: Request }) {
  ...
  const body = await request.json().catch(() => null) as { prompt?: string; preferredType?: string } | null;
  ...
  constraints.push(`Workspace: ${totalModules} module(s) (${publishedModules} published, ${drafts} draft). Avoid names already in use.`);
  ...
  const band = confidence >= CONFIDENCE_THRESHOLDS.DIRECT ? 'direct'
    : confidence >= CONFIDENCE_THRESHOLDS.WITH_ALTERNATIVES ? 'with_alternatives' : 'low';

  const jobs = new JobService();
  const job = await jobs.create({
    shopId: shopRow.id,
    type: 'AI_GENERATE',
    payload: { promptLen: prompt.length, classifiedType: classification.moduleType, intent: intentPacket.classification.intent, source: 'agent_api' },
  });
