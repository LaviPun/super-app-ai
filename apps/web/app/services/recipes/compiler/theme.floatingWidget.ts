...
export function compileThemeFloatingWidget(
  spec: Extract<RecipeSpec, { type: 'theme.floatingWidget' }>,
  target: Extract<DeployTarget, { kind: 'THEME' }>
): CompileResult {
  if (!target.moduleId) {
    throw new Error('theme.floatingWidget requires moduleId (publish via app extension).');
  }
  ...
  return {
    ops: [{ kind: 'AUDIT', action: 'compile.theme.floatingWidget', details: JSON.stringify({ moduleId: target.moduleId }) }],
