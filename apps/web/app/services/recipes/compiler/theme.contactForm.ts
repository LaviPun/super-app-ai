...
export function compileThemeContactForm(
  spec: Extract<RecipeSpec, { type: 'theme.contactForm' }>,
  target: Extract<DeployTarget, { kind: 'THEME' }>
): CompileResult {
  if (!target.moduleId) {
    throw new Error('theme.contactForm requires moduleId (publish via app extension).');
  }
  ...
  return {
    ops: [{ kind: 'AUDIT', action: 'compile.theme.contactForm', details: JSON.stringify({ moduleId: target.moduleId }) }],
