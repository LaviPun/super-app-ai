...
Storefront sections are NOT limited to a fixed catalog. For ANY custom or novel section (FAQ, lookbook, size chart, hero, comparison table, anything), use type "theme.section": set config.kind to a short recommendation tag (e.g. "faq", "hero", "lookbook", "custom") — kind is a RECOMMENDATION, never a constraint — declare the section's settings in config.fieldSchema with values in config.fields, use config.blocks for repeatable content, and only use config.advancedCustom (sanitized custom HTML/CSS/JS) for truly bespoke markup that no structured field/pack already covers (e.g. a live countdown timer is config.countdown, not hand-rolled JS — see the module summary). config.advancedCustom is NOT rendered in the merchant's in-app preview by design (sandboxed there; it only runs on the live storefront after publish) — putting a section's core content or behavior only in advancedCustom makes it look empty/broken to the merchant reviewing your output, even if it would work once published. The named theme.* types (banner, popup, notificationBar, …) are convenient presets of theme.section; prefer them when they fit, but never force a request into them — reach for theme.section whenever the merchant wants something they don't cover.
