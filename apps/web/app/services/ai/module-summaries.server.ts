...
Controls: prefer declaring fieldSchema + fields for structured content; use blocks for repeatable items; use advancedCustom only for truly bespoke markup that no structured field/pack below already covers. IMPORTANT: config.advancedCustom.customHtml/customJs is NOT executed in the merchant's in-app preview (sandboxed there by design) — it only runs on the live storefront after publish, so building a whole section's primary content (headline, CTA, a live timer, etc.) through advancedCustom makes it look broken/empty to the merchant reviewing it. Always populate the matching structured fields/blocks/packs too, even if you also add bespoke HTML/JS. kind is a recommendation, never a constraint.
Live countdown timer on ANY kind (not just popup — notification-bar/band/hero/launch can all carry it too): set config.countdown = { enabled:true, mode('fixed'|'evergreen'|'daily'|'session'), endAt(ISO 8601, required for mode 'fixed'), durationMinutes(mode 'evergreen'), onExpire('hide'|'freeze'|'restart'), timerStyle('plain'|'tiles'), label, labels{days/hours/minutes/seconds} }. This is a real, rendered live timer (preview + storefront) — always prefer it over hand-rolling countdown markup/JS in advancedCustom.
Overlay/popup kinds (activation:'overlay', kind:'popup') add: title(str 1-60), body(str 0-240), ctaText/ctaUrl, secondaryCtaText/secondaryCtaUrl, trigger(ON_LOAD|ON_EXIT_INTENT|ON_SCROLL_25|ON_SCROLL_50|ON_SCROLL_75|ON_CLICK|TIMED), delaySeconds(0-300), frequency(EVERY_VISIT|ONCE_PER_SESSION|ONCE_PER_DAY|ONCE_PER_WEEK|ONCE_EVER), maxShowsPerDay(0-100), showOnPages(ALL|HOMEPAGE|COLLECTION|PRODUCT|CART|CUSTOM), customPageUrls(str[] max 20), autoCloseSeconds(0-300), showCloseButton(bool), countdownEnabled(bool), countdownSeconds(0-86400), countdownLabel(str max 40) — or the richer config.countdown pack above for mode/onExpire/timerStyle control.
