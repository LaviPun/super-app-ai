...
**Status**: **Partial** — `@superapp/security` SSRF module shipped on `master`
...
| Shipped | `packages/security` with `assertSafeTargetUrl` SSRF helper (`ssrf.ts`), unit tests, and integration in connector worker handler |
| Pending | App Store readiness review, secrets audit, policy gates |
