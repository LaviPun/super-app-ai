...
Running **Qwen3-class** models efficiently usually means **vLLM**, **llama.cpp**, or **Ollama** on GPU elsewhere (Modal GPU containers, RunPod, or self-hosted Docker). The reference router stays a small HTTP service that calls an OpenAI-compatible inference URL (`ROUTER_OPENAI_BASE_URL`, `ROUTER_BACKEND=openai`). This Modal app scales **HTTP ingress** while inference stays pinned to whatever backend you operate.
