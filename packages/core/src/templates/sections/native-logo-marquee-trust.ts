...
export const NATIVE_LOGO_MARQUEE_TRUST_TEMPLATES: TemplateEntry[] = [
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'logo-marquee', 'trust', 'social-proof', 'index', 'marquee'],
    spec: {
      type: 'theme.section',
      name: 'Brand Logo Marquee — Scrolling',
      category: 'STOREFRONT_UI',
  ...
  },
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'press', 'as-seen-in', 'trust', 'social-proof', 'index'],
    spec: {
      type: 'theme.section',
      name: 'As Seen In — Press Logo Row',
      category: 'STOREFRONT_UI',
  ...
  },
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'trust-badges', 'payment', 'security', 'cart', 'product'],
    spec: {
      type: 'theme.section',
      name: 'Payment & Security Trust Marks',
      category: 'STOREFRONT_UI',
  ...
  },
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'trust-badges', 'guarantee', 'shipping', 'returns', 'product'],
    spec: {
      type: 'theme.section',
      name: 'Guarantee Icons — Shipping & Returns Strip',
      category: 'STOREFRONT_UI',
  ...
  },
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'logo-marquee', 'stockists', 'trust', 'grid', 'page'],
    spec: {
      type: 'theme.section',
      name: 'Framed Retailer Logo Grid',
      category: 'STOREFRONT_UI',
  ...
  },
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'trust', 'stats', 'social-proof', 'number-ticker', 'index'],
    spec: {
      type: 'theme.section',
      name: 'Trust Stat Strip — Big Numbers',
      category: 'STOREFRONT_UI',
  ...
  },
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'trust-badges', 'certification', 'compliance', 'product'],
    spec: {
      type: 'theme.section',
      name: 'Certification & Compliance Badges',
      category: 'STOREFRONT_UI',
  ...
  // 08 — V-A A2: payment-methods strip using the curated inline-SVG badge-icon
  // catalog (blocks[].fields.icon → a namespaced <symbol> in the theme sprite). Icon
  // ids: visa/mastercard/amex/paypal/shop-pay/apple-pay/google-pay/klarna.
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'payment-badges', 'payment', 'icons', 'cart', 'product'],
    spec: {
      type: 'theme.section',
      name: 'Payment Methods Strip',
      category: 'STOREFRONT_UI',
      requires: ['THEME_ASSETS'],
      config: {
        kind: 'payment-badges',
        ...
        subtitle: 'All major cards and wallets accepted',
        layout: { layout: 'stacked' },
        fields: { badgesPerRowDesktop: 8, badgesPerRowMobile: 4 },
        blocks: [
          { kind: 'badge', text: 'Visa', fields: { icon: 'visa' } },
          { kind: 'badge', text: 'Mastercard', fields: { icon: 'mastercard' } },
          { kind: 'badge', text: 'Amex', fields: { icon: 'amex' } },
          { kind: 'badge', text: 'PayPal', fields: { icon: 'paypal' } },
          { kind: 'badge', text: 'Shop Pay', fields: { icon: 'shop-pay' } },
          { kind: 'badge', text: 'Apple Pay', fields: { icon: 'apple-pay' } },
          { kind: 'badge', text: 'Google Pay', fields: { icon: 'google-pay' } },
          { kind: 'badge', text: 'Klarna', fields: { icon: 'klarna' } },
        ],
      },
      placement: { enabled_on: { templates: ['product', 'cart', 'page'] as (typeof THEME_PLACEABLE_TEMPLATES)[number][] } },
      style: {
        layout: { mode: 'inline', anchor: 'top', offsetX: 0, offsetY: 0, width: 'container', zIndex: 'base' },
        spacing: { padding: 'medium', margin: 'none', gap: 'medium', density: 'comfortable' },
  ...
  // 09 — V-A A2: trust/guarantee reassurance row using the catalog's trust glyphs.
  {
    ...
    type: 'theme.section',
    icon: 'trust',
    tags: ['section', 'trust-badges', 'guarantee', 'returns', 'shipping', 'product'],
    spec: {
      type: 'theme.section',
      name: 'Trust Guarantee Row',
      category: 'STOREFRONT_UI',
      requires: ['THEME_ASSETS'],
      config: {
        kind: 'trust-badges',
        ...
        subtitle: 'Every order is protected',
        layout: { layout: 'stacked' },
        fields: { iconsPerRowDesktop: 4, iconsPerRowMobile: 2 },
        blocks: [
          { kind: 'badge', text: 'Secure checkout', fields: { icon: 'secure-checkout', caption: 'SSL-encrypted payment' } },
          { kind: 'badge', text: 'Free returns', fields: { icon: 'free-returns', caption: '30-day, no questions asked' } },
          { kind: 'badge', text: 'Fast shipping', fields: { icon: 'fast-shipping', caption: 'Dispatched in 24 hours' } },
          { kind: 'badge', text: 'Money-back guarantee', fields: { icon: 'money-back', caption: 'Full refund within 30 days' } },
        ],
      },
      placement: { enabled_on: { templates: ['product', 'page'] as (typeof THEME_PLACEABLE_TEMPLATES)[number][] } },
      style: {
