...
- **Transitional exceptions:** `generate._index.tsx` still renders through the vendored system (dedicated migration pending); the vendored CSS therefore remains globally linked in root.tsx until that lands, after which it moves to the internal layout's `links()`. `@shopify/polaris` (React) is no longer imported by any merchant file; it remains a dependency only for root's internal-branch `PolarisProvider`.
