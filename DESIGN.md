...
- **Merchant surface (embedded app) — Polaris web components ONLY (owner decision 2026-07-15):** every merchant route renders `<s-page>`/`<s-section>`/`<s-*>` elements (script: `cdn.shopify.com/shopifycloud/polaris.js`, loaded in root.tsx's embedded branch; typings via `@shopify/polaris-types` in `app/global.d.ts`). Merchant-only light-DOM helpers (compact StatTile with inline sparkline, KV, Progress, Tabs, SubnavTabs, ConfirmModal, EmptyState, charts) live in `apps/web/app/components/merchant/polaris.tsx` + `apps/web/app/styles/merchant.css` (`sa-m-*` classes, fixed admin-matched values, token-free). Merchant pages must NOT import `~/components/superapp` or reference `--sa-*`/vendored `--p-*` tokens. **One narrow, deliberate exception:** `MerchantShell` itself (not merchant pages) mounts the shared `CommandPalette`/`Toast` from `~/components/superapp` for ⌘K + toasts; their styling is ported into `merchant.css` under a `.sa-merchant`-scoped block (same class names as the vendored version, fixed admin-matched literal values, still token-free — no `--p-*`/`--sa-*` custom properties) so it can't reach internal admin's identically-named vendored rules. KPI style: dense, compact, graph-inline — no oversized stat cards or dead space.
- **Internal admin — vendored SuperApp design system (unchanged):** `internal.*` keeps `apps/web/app/styles/superapp/*.css` + `app/components/superapp/*` exactly as before.
- **Do NOT add `@shopify/app-bridge-types`:** it augments React's global `ButtonHTMLAttributes` and breaks the vendored `Btn`.
- **Component preference order (merchant):** Polaris web component -> `merchant/polaris.tsx` helper -> new helper in that file (never a one-off in a route).
- **No remaining transitional exceptions (closed 2026-08, WS-F Tasks 14–15):** `generate._index.tsx` — the last merchant route still on the vendored system — is migrated; every merchant route now renders Polaris web components only. The vendored CSS (`polaris.css`/`shell.css`/`pages.css`) is no longer globally linked in `root.tsx`; it moved to `internal.tsx`'s own `links()` and is internal-admin-only. `generate.css` (exclusively `generate._index.tsx`'s CSS) was deleted outright. `@shopify/polaris` (React) is no longer imported by any merchant file; it remains a dependency only for root's internal-branch `PolarisProvider`.
...
- **Shared foundation** — `apps/web/app/components/superapp/`: `icons.tsx`, `ui.tsx` (Btn, Badge, StatusBadge, Card, DataTable, Modal, Field, Input, Select, Toggle, Tabs, Banner, EmptyState, Toast, PageHead, FilterBar, StatTile, …), `page-helpers.tsx` (charts + `fmtCents/fmtNum/fmtMs/titleCase`), `store-utils.ts` (storeHealth/exportCSV/navCounts), `CommandPalette.tsx` (⌘K), `MerchantSubnav.tsx`. (The former `placeholder-data.ts` deterministic-placeholder layer was removed in the 2026-07 repair pass — every admin/merchant surface now renders real backend data or an honest `EmptyState`.)
...
| 2026-07-03 | **Adopt the OKLCH design-token system** for generated storefront modules (phase #2, `specs/029-styling-tokens/`), evolving the token grammar toward the `028` research design-vocabulary — **user-approved deviation** from the prior coarse-enum grammar. **Increment 2A (done):** `StorefrontStyle` gains motion tokens (`--sa-motion` / `--sa-ease` / `--sa-motion-ambient`), the four coherent **elevation idioms** (`--sa-elevation`: soft/glow/border/emboss), a scale-aware **radius ladder** (`--sa-radius-sm/md/lg/xl` + `shape.scaling`), and a **density** dial — all additive/optional, compiled in `style-compiler.ts`. The OKLCH seed-derived 12-step semantic color ramp + `-content` pairing follow in 2B. | Coarse-enum `StorefrontStyle` (`padding: small\|medium\|large`, 5 flat hexes) couldn't carry the token grammar the 6 packs describe → below-parity visuals. Widening it to the research tokens makes generation token-driven and YC-tier. Spec: `specs/029-styling-tokens/plan.md` + `028/research/design/design-vocabulary.md`. |
| 2026-07-15 | **Merchant surface migrated to Polaris web components** ("Polaris only" owner decision): all 26 merchant routes off the vendored system / Polaris React onto `<s-*>` (incl. SchemaForm + FlowBuilder rewrites, the 7 provider-less Polaris React routes, support CRM pages); dense KPI directive (compact tiles + inline sparklines, Linear/Stripe register); `MerchantShell` slimmed to SubnavTabs + App Bridge toast + ⌘K. Pending: `generate._index` (still vendored, own phase), then CSS relocation to internal layout. | Merchant app must look/behave native in Shopify admin; audit found 3 coexisting UI systems and a provider-less Polaris React bug. Docs-spec'd plan quotas also aligned (modulesTotal 3/20/100/1000) in code + DB. |
| 2026-07-14 | **Impeccable hardening pass on the Internal Admin** (PRODUCT.md at `apps/web/PRODUCT.md`): implemented the dark theme the Color section committed to (full `--p-*`/`--sa-*` override under `@media (prefers-color-scheme: dark)` in `styles/superapp/polaris.css`, AA-targeted, brand-hue-tinted neutrals); added a global `prefers-reduced-motion` branch; made the admin shell mobile-responsive (off-canvas nav drawer + hamburger under 860px); moved fonts off a render-blocking `@import` to preconnect + parallel `<link>` in `root.tsx`; added shell focus-visible + `<nav>`/aria landmarks; fixed dashboard delta-color polarity, guarded the loader per-query + `ErrorBoundary`, and made list/table rows real keyboard-operable links. | Critique (28/40) + audit (14/20) of the internal admin found a committed-but-unimplemented dark mode, no reduced-motion/mobile shell, and several a11y/robustness gaps. **Dark theme is implemented but still needs a visual QA pass** (couldn't render the SSO+DB-gated app headlessly). |
