...
## §23 — Basic-plan bundle pricing: Plus-only cart-transform `lineUpdate` → plan splitter + managed discount (Task 6, 2026-07-14)

**Root cause.** Fixed-price bundles were priced by the cart-transform Function's `lineUpdate` operation (the only channel that sets an absolute per-unit price on a merged line). `lineUpdate` runs **only on Shopify Plus/Enterprise and dev stores** — on Basic/Grow/Advanced it is a no-op, so a non-Plus merchant's bundle merged visually but never reprice­d to the configured amount. Dev stores mask this because they satisfy the Plus gate natively.

**Fix.** A pure, plan-aware splitter `splitBundlePricingForPlan(bundles, plan)` (`apps/web/app/services/bundles/bundle-pricing-split.ts`), wired into the publish path at `blueprint.service.ts:~471-481` with the plan from `CapabilityService.getPlanTier` (reads `Shop.planTier`). On Plus/Enterprise it keeps the legacy lineUpdate config. On non-Plus it:
- strips the single `fixed-price` from the cart-transform config → the wasm takes the **merge-only** path (no lineUpdate), and
- emits a companion rule `{ id: "bundle:<id>", when: { skuIn: ["superapp-bundle-<bundleId>"] }, apply: { fixedPricePerUnit } }` targeting the **parent bundle variant's SKU** (the merged line's merchandise). Cart transform runs before discount functions, so the discount sees the merged line and reduces it to `fixedPricePerUnit × qty`. Percentage bundles are untouched (`percentageDecrease` merge is plan-universal); tiered fixed-price stays Plus-only (post-merge quantity can't reflect component count).

**Where the managed rules live.** `BundleProductService.writeBundlePricingRules` (`bundle-product.service.ts`) merges the `bundle:*` rules into the `discountRules` function config — the `config_json` field of the `$app:superapp_function_config` metaobject with handle `superapp-fn-discountRules`, the same object the `superapp-discount` wasm reads. Managed (`bundle:`-prefixed) rules are replaced on each publish; module-authored rules are preserved. `ensureAutomaticBundleDiscount` idempotently ensures the automatic app discount node titled **"SuperApp Bundle Pricing"** that activates the discount Function.

**Two live-API fixups found during Task 6 e2e (API 2026-04, unified Discounts).** The fallback was inert on a real store until these:
1. **Discount-function lookup.** `ensureAutomaticBundleDiscount` searched `shopifyFunctions` for `apiType === 'product_discounts'` (the legacy type). On 2026-04 the `superapp-discount` function (target `cart.lines.discounts.generate.run`) reports `apiType: "discount"`, and the shop carries a *second* `discount`-type function (`superapp-shipping-discount`). Lookup now keys off the stable extension **handle `discount-function`** (`DISCOUNT_FUNCTION_HANDLE`). Without this the node create threw "no product_discounts function found".
2. **`discountClasses` required.** `discountAutomaticAppCreate` on a `discounts`-apiType function requires `discountClasses`. Added `discountClasses: ['PRODUCT']` (the function generates per-line/product discounts). Without this the create returned userError "Functions configured to use the `discounts` API type require the discountClasses field to be set."

Also switched the idempotency lookup from `automaticDiscountNodes` (search-index backed, lags several seconds after create → a back-to-back republish would miss the fresh node and hit "Title must be unique") to the write-consistent `discountNodes` connection.

**How verified.** Deployed `super-app-ai-20` (`shopify app deploy --allow-updates`; both `cart-transform-function` + `discount-function` wasm built). Forced the fallback by setting `Shop.planTier='BASIC'` on `kushtestinfotech.myshopify.com` (restored to `FREE` after). Ran the **real** services (`ensureParentBundleProduct` → `splitBundlePricingForPlan('BASIC')` → `activateCartTransform` → `ensureAutomaticBundleDiscount` → `writeBundlePricingRules`) against the live store via a throwaway Vitest driver using the offline Admin token, then queried Admin GraphQL. All three artifacts present and cross-consistent for a $99.99 fixed-price bundle:
- **A1** `CartTransform/105316543` `$app:bundle_config` — our bundle entry present, `price` key **absent** (merge-only, correct for non-Plus).
- **A2** `Metaobject/230600540351` (`superapp-fn-discountRules`) `config_json.rules` contains `bundle:task6-verify-bundle`, `when.skuIn=["superapp-bundle-task6-verify-bundle"]`, `apply.fixedPricePerUnit=99.99`.
- **A3** `DiscountAutomaticNode/1475593273535` — `DiscountAutomaticApp` "SuperApp Bundle Pricing", `status: ACTIVE`, `discountClasses: [PRODUCT]`, `appDiscountType.functionId` = SuperApp Discount, `targetType: LINE_ITEM`.
