...
Two operational details (both PR #43): Anthropic responses' `cache_read_input_tokens`/`cache_creation_input_tokens` are recorded as `cacheReadTokens`/`cacheCreationTokens` in `AiUsage.meta`, so Anthropic cache hit rates are observable per call even while the flag is off (they'll simply read 0/absent). And structured-output forced-tool names are **index-invariant** — the per-option `_${idx}`/`_qa` suffixes were dropped so all fan-out option calls (and QA regens) for a moduleType share one `tools` block and therefore one cache lineage; nothing downstream matches on the tool name.
...
Cost attribution: usage is attributed to the provider that **actually served** the request. Each `ConfiguredLlmClient` stamps `servedProviderId` on its result, so when a later leg of the chain serves, `attributeServedCost` keys the price lookup on (served providerId, served model) and records the `AiUsage` row under that provider — not the one first tried. Env-key clients (no DB row) carry no `servedProviderId` and price at 0, as before. The served model is also recorded in `AiUsage.meta`. A model with **no `AiModelPrice` row** also prices at 0 — since PR #45, `estimateCostCentsFromDbRates` logs a WARN naming the unpriced model (once per model per process, pointing at `seed:ai-pricing`/the ai-providers admin) instead of masking real spend silently.

**Eval-mode isolation (PR #42):** the eval harness must score exactly the model it claims to score, so `getLlmClient()` accepts eval-only options `disableFallback` (skips the env OpenAI wrap, the operator manual fallback, and cost-chain legs) and `ignoreEnvSkills` (keeps `ANTHROPIC_SKILLS` env out of eval request shapes). `EVAL_PROVIDER_ID=<AiProvider id or name>` pins that DB row via `ConfiguredLlmClient` and **fails fast** if it doesn't exist; `EVAL_PROVIDER_ID=env` explicitly selects the env-key client. Production request behavior is unchanged — see [`docs/testing.md`](./testing.md) §4 for running the harness.

**Daily spend cap (observability only, PR #46):** `services/observability/ai-spend-guard.server.ts` compares today's summed `AiUsage.costCents` against a soft daily cap — `AppSettings.aiDailySpendCapCents` → `AI_DAILY_SPEND_CAP_CENTS` → **$20 default** (`DEFAULT_DAILY_CAP_CENTS = 2000`). Breaches fire `AI_SPEND_CAP_EXCEEDED` through `OpsAlertService` and surface in `/healthz/deep` and the internal-admin ops banner; the guard **never blocks a request**.
...
- **Code execution**: Pass the `code_execution_20260521` tool (the client sent the stale `code_execution_20250825` type until PR #43 fixed the drift) and the code-execution beta header.

Note (P2-A, PR #43): enabling code execution on a provider disables structured-output tool-forcing for its calls (the two are mutually exclusive in `anthropicGenerateRecipe`) — `guardAnthropicSkillsConfig` (`llm.server.ts`) logs an `[ai-skills]` warning whenever that trade-off is active, so the quietly-degraded JSON path is visible in logs instead of silent.
...
**Per-type recipe budgets** (`apps/web/app/services/ai/token-budget.server.ts`) override the generic default per module type. Notably (generation-quality pass, PR #41): `theme.section` **7000** and `proxy.widget` **5500** — these two carried the *smallest* budgets in the system (3000/2500) despite the richest design-system prompt, reproducibly truncating every parallel option (`stop_reason=max_tokens`) on real prompts. Check the file, not this doc, for the full current map.

**Delta (Tier-1 template-edit) budget** (PR #38): `getDeltaTokenBudget` is **0.75×** the type's recipe budget (was 0.5×, which truncated every delta call on Sonnet-5-class models and silently fell back to freeform), floored at `MIN_DELTA_BUDGET` (1200). On a `TruncatedOutputError`, `generateRecipeViaDelta` retries exactly once with a 1.5× bumped budget (capped 24000) before rethrowing — mirroring hydration's budget-bump pattern; the retry claims no extra billable unit.
