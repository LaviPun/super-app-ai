...
| Env / setting | Use |
|---------------|-----|
| **`INTERNAL_AI_ROUTER_URL`** (Remix `.env`) | HTTPS origin of **`superapp-internal-ai-router-proxy`** only (`modal deploy modal_app.py`), or the Railway router URL directly. Secret **`INTERNAL_ROUTER_UPSTREAM_URL`** on Modal must target your **real** upstream router (Railway, local Node, or Docker), not the optional mock. |
