...
- **AI Providers** — configured providers show a server-computed **prefix+last-4 key preview** (e.g. `sk-ant-…cAAA`, never the middle) with a **Show/Hide reveal toggle** (PR #33): the full key is only decrypted and sent to the client on demand via the `revealApiKey` action (behind `requireInternalAdmin`), and every reveal writes an `AI_PROVIDER_KEY_REVEALED` audit row; for **Claude (ANTHROPIC)** you can set Agent Skills (e.g. pptx, xlsx) and enable code execution; model pricing table
...
**Live-tail reconnect behavior (PR #40).** The Activity, API Logs, and Support tails all share `~/hooks/useLiveTail` (React wrapper over the pure `attachLiveTail` core). On a transient SSE drop (proxy timeout, backend restart — `EventSource` stays at `CONNECTING` and auto-retries), the UI shows a **"reconnecting"** banner instead of silently claiming "Live tail active" with no rows (the old code only reacted to `readyState === CLOSED`, so a dropped stream hung blank forever). After 5 consecutive failed retries it gives up loudly — closes the connection, toasts, flips the toggle off. Covered by `app/__tests__/use-live-tail.test.ts`.

**Detail routes are de-nested (PR #40).** The Activity/API Logs/Jobs/Error Logs/Webhooks detail routes are named with Remix's trailing-underscore convention (`routes/internal.X_.$id.tsx`) to opt out of the parent list route's layout nesting — the list pages render their full UI unconditionally with no `<Outlet/>`, so under the default nesting a detail navigation fetched data and changed the URL but rendered nothing (the owner-reported "logs are not opening" bug). URLs are unchanged. The job detail page's **Replay** button posts through the audited `/internal/ops` `job_replay` intent like the list page (it previously posted to `/internal/jobs`, which has no action — a full-page crash).
