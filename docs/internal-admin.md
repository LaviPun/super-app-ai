...
- **Email and Slack** — gated by a rolling-window failure count. Both fire only once `AppSettings.opsAlertThresholdCount` alerts of the same kind have occurred within `opsAlertThresholdWindowMin` minutes, tracked via an `ActivityLog`-backed count (`OPS_ALERT_FIRED` rows), so a single blip doesn't page anyone but a sustained failure does.
