...
| Flow (app as connector) | `FLOW_EXTENSION_KINDS`, `FLOW_AUTOMATION_TRIGGERS`, `FLOW_STEP_KINDS` | `flow.automation` runs through `FlowRunnerService` (a live linear runtime) but the workflow-definition publish path proper is still pending, so the eligibility registry marks it `needs_runtime` — pessimistic relative to what actually executes; re-verify against `extension-eligibility.ts` before repeating this claim, since it's exactly the kind of split-state fact that drifts. |
...
- **`admin.discountUi` is `needs_runtime`** — the Spring-2026 discount-details extension it targets is not yet built.
