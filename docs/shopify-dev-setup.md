...
Publish preflight (`classifyModulePublishability`) **fails loudly** when a function type has no deployed runtime behind it, naming the required handle. Republish upserts the config metaobject in place (`computeRepublishDiff`); unpublish deletes it.

**Publish model (updated 2026-07-03).** The preflight status enum is **`deployable | needs_runtime`** — the old `gated`/`blocked` split collapsed into `needs_runtime` (the eligibility source of truth is now the **core eligibility registry**, not a hardcoded list). `PublishService.publish` runs the gate before any deploy work and throws `ModuleNotPublishableError` when `!willDeploy` (surfaced by `api.publish.tsx` as HTTP `422`, and shown pre-publish in the merchant Builder's validation tab), so no caller can report a non-deploying module as "published". `analytics.pixel` is now **deployable** via a real `WEB_PIXEL_UPSERT` op (`web-pixel.service.ts`).

**Deployed-extension source of truth.** `services/publish/deployed-extensions.server.ts` returns **manifest ∪ env**: the Function handles checked into `extensions/` (real `shopify.extension.toml`, `type = "function"`) UNION any declared in **`SHOPIFY_DEPLOYED_FUNCTION_EXTENSIONS`** (comma/space-separated). Because the manifest is populated from what's actually built, functions are deployable out of the box; the env var only adds extras for environments where more are shipped. Layer-b config writes call `MetaobjectService.getFunctionConfigByKey` + `computeRepublishDiff` to skip no-op republishes (metaobjects are handle-keyed, so republish never duplicates).

Representative smoke path (dev store): `theme.section`, one function (e.g. `functions.discountRules` with its extension deployed), and a `flow.automation` (`needs_runtime`) — generate → preview → publish/preflight → verify → republish → rollback.
