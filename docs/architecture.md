...
| `worker` | `apps/web/railway.worker.toml` | `pnpm --filter web worker:start` (`scripts/worker.ts`) | Connects to the queue Redis via `@superapp/job-orchestration` and serves `/healthz`. As of this writing it is a WS-A skeleton: it proves the Redis/service wiring but does not yet run real BullMQ job handlers — `JOB_EXECUTION_MODE` stays `inline` (work runs synchronously in the `web` process) until WS-C's async engine lands. **Pending (unmerged):** WS-C wires real BullMQ workers into this process. |
