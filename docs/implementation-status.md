...
- **Vendored design system:** `apps/web/app/styles/superapp/{polaris,shell,pages,generate}.css` (Polaris-aligned `--p-*`/`--sa-*` tokens — NOT `@shopify/polaris`; brand tokens match [DESIGN.md](../DESIGN.md)) + shared `apps/web/app/components/superapp/*` (icons, ui primitives, page-helpers/charts, `placeholder-data.ts`, `CommandPalette` ⌘K, `MerchantSubnav`).
