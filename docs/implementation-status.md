...
- **Vendored design system:** `apps/web/app/styles/superapp/{polaris,shell,pages,generate}.css` (Polaris-aligned `--p-*`/`--sa-*` tokens — NOT `@shopify/polaris`; brand tokens match [DESIGN.md](../DESIGN.md)) + shared `apps/web/app/components/superapp/*` (icons, ui primitives, page-helpers/charts, `store-utils` real-data helpers, `CommandPalette` ⌘K, `MerchantSubnav`). *(The former `placeholder-data.ts` layer was removed in the 2026-07 repair pass — surfaces render real data or honest EmptyStates.)*
- **Internal Admin:** `internal.tsx` swapped Polaris `Frame` → `AdminChrome` (collapsible rail: Overview · Operations · Platform · AI & Models · Catalog; top-bar ⌘K palette; health footer). New Platform pages: `internal.modules.*`, `internal.flows.*`, `internal.connectors.*`, `internal.data-stores.*`, `internal.customers.*`, plus `internal.jobs.$jobId`, `internal.webhooks.$webhookId`, `internal.recipe-edit.$id`. All admin mutations route through the new shared **`/internal/ops`** action (`useAdminOps()` hook) → real service call + `ActivityLogService` audit entry. Auth/loaders unchanged.
- **Merchant Dashboard:** App Bridge `<s-app-nav>` set to **Dashboard · Build · Insights · Settings · Billing** (outside the app); `components/merchant/MerchantShell.tsx` wraps each page with the design frame + `MerchantSubnav` (Build → Modules/Flows/Connectors/Data/Templates; Insights → Analytics/Activity) + ⌘K + toasts. New routes: `generate._index`, `templates._index`, `analytics._index`, `activity._index`, `billing.history`, `help._index`.
- **Verification:** typecheck 0 errors; lint passes (73 warnings, under cap); internal Playwright smoke 35/35; merchant smoke 2/2; both surfaces visually verified 1:1.
...
- **Main code-gen chatbot — third provider (Gemini).** New [`gemini.client.server.ts`](../apps/web/app/services/ai/clients/gemini.client.server.ts) (Generative Language API `:generateContent`, `x-goog-api-key`, native `responseSchema`). `GEMINI` added to `ProviderKind`, `ConfiguredLlmClient` dispatch, env path (`GEMINI_API_KEY` / `GEMINI_DEFAULT_MODEL`, default `gemini-2.5-flash`), and the `/internal/ai-providers` dropdown. Live-validated endpoint path (400 `API_KEY_INVALID`, not 404).
- **Default + fallback selection.** `AppSettings.fallbackAiProviderId` (new nullable column) lets an operator assign a fallback provider on `/internal/ai-providers` (*Default & fallback AI* card). `getLlmClient` now wraps the active (default) provider and the fallback in `FallbackLlmClient` for the DB path too — previously only the env path had failover. Default provider = the active `AiProvider`.
- **Cost/usage already present:** per-request cost + tokens (`/internal/usage`, `AiUsage`), token rates (`AiModelPrice`), per-provider balance/limits/spend (`/internal/ai-accounts`). Gemini providers populate these once their `AiModelPrice` rows are entered.
- **Backend:** `PLATFORM_BACKEND=cloudflare` recommended (per prior entry).
- **Docs:** [`ai-providers.md`](./ai-providers.md) (Gemini client + Default & fallback section), [`internal-admin.md`](./internal-admin.md) (internal self-hosted default), `.env.example` (`GEMINI_*`).
- **Verification:** `apps/web` vitest **498 passing** / 16 skipped (incl. new Gemini client + Anthropic-backend tests); `tsc --noEmit` clean; `prisma db push` applied `fallbackAiProviderId`.
...
## 2026-06-12 (Global audit — merge-damage repair, AI chat fixes, dead code)

- **Contracts repaired (merge `9338a85` damage):** `vr/v2` merge dropped the master-side `jobs.ts` image-worker constants and the full `index.ts` export surface. Restored as [`image-worker-jobs.ts`](../packages/platform-contracts/src/image-worker-jobs.ts); `index.ts` now exports all eight modules. Same-name conflicts resolved explicitly: platform (CF-queue) event renamed `PlatformWorkerEventSchema`/`PlatformWorkerEvent`; platform flow payload renamed `FlowRunWorkerPayloadSchema`; duplicate `JobExecutionMode` removed from `platform-jobs.ts` (canonical in `rollout-cutover.ts`). Legacy BullMQ `WorkerEvent`/`FlowRunPayloadSchema` (in `jobs.ts`) unchanged.
- **API routes mounted:** `routes/jobs.ts` (now `registerPlatformJobRoutes`) and `routes/preview.ts` were never mounted in `buildApp`; both registered. `GET /v1/jobs/:jobId` checks the BullMQ store first, then falls back to the platform job status store.
- **Internal AI assistant (admin chat):** chat target timeout now floors at 30s (`INTERNAL_AI_CHAT_TIMEOUT_MS` overrides) — the shared 3000ms router-target default aborted Ollama cold starts (~3.8s TTFB measured). Modal proxy (`deploy/modal-qwen-router/modal_app.py`) gained chat passthrough (`/v1/chat/completions`, `/chat/completions`, `/api/chat`, `/api/tags`) so the cloud target stops 404ing; redeploy with `modal deploy modal_app.py`.
- **Dead code:** removed `packages/security/src/ssrf.ts` + test (duplicate of `@superapp/network-security`; facade index re-exports). Unreferenced but spec-linked (left in place, decision pending): `connector-worker.server.ts`, `publish-worker.adapter.server.ts`, `tolerant-json.server.ts`, `preview-artifact-store.server.ts`, `module-events.server.ts` (all under `apps/web/app/services/`).
- **Verification:** `pnpm test:v2:typecheck` 0 errors; `pnpm test:v2:unit` all green (api 41, workers 46, contracts 30, core 125, db 13, frontend 15); `pnpm test:v2:build` clean; `apps/web` vitest 493 passed / 16 skipped. `better-sqlite3` recompiled for Node 24.
- **Open conflict (decision needed):** two job/queue generations coexist — BullMQ/Fastify (`QueueName`, `WorkerEvent`) vs Cloudflare platform queues (`PlatformQueueName`, `PlatformWorkerEvent`). Now explicitly named, but consolidation per spec 021 cutover is still pending.
...
- **Decryption / parse surfacing.** [`router-runtime-config.server.ts`](../apps/web/app/services/ai/router-runtime-config.server.ts) now returns `{ config, parseError? }` from `getRouterRuntimeConfig`. `/internal/model-setup` renders a red decryption banner; `/internal/ai-assistant` ([`internal.ai-assistant.tsx`](../apps/web/app/routes/internal.ai-assistant.tsx)) renders a matching warning chip linking back to model-setup.
- **Auto-reprobe.** New resource route [`/internal/ai-assistant/probe`](../apps/web/app/routes/internal.ai-assistant.probe.tsx) returns `{ localMachine, modalRemote, parseError? }`. The assistant page polls it every 20s with `AbortController` while the preferred target's `chatProbe.ok === false` and exposes a manual "Recheck" button.
- **Tightened SSRF guard.** `assertSafeTargetUrl` in [`internal-assistant.server.ts`](../apps/web/app/services/ai/internal-assistant.server.ts): `http://` only accepts exact `127.0.0.1`/`localhost`/`::1`; `https://` rejects link-local IPv4 (`169.254.*`), IPv6 link-local (`fe80::`), and known cloud metadata hosts. New env `INTERNAL_AI_ALLOW_HOSTS` (comma-separated) is the opt-in allowlist for trusted internal HTTPS hosts.
- **SQLite self-heal guard.** [`internal-assistant-store.server.ts`](../apps/web/app/services/ai/internal-assistant-store.server.ts) wraps `PRAGMA`-style self-heal in `try/catch` and no-ops on non-SQLite errors with a single info log, so the path is safe on Postgres.
- **Router auth in prod.** [`internal-ai-router.ts`](../apps/web/scripts/internal-ai-router.ts) ignores `ROUTER_REQUIRE_AUTH=0` when `NODE_ENV=production` and logs `[internal-ai-router] WARN: ROUTER_REQUIRE_AUTH=0 ignored in production`.
- **Session delete + audit retention.** `/internal/ai-assistant` adds an `intent: 'deleteSession'` flow with confirm; messages cascade-delete while `InternalAiToolAudit` rows are preserved via `ON DELETE SET NULL`. Daily cron in [`api.cron.tsx`](../apps/web/app/routes/api.cron.tsx) calls [`internal-ai-audit-retention.job.ts`](../apps/web/app/services/jobs/internal-ai-audit-retention.job.ts) to purge audits older than `INTERNAL_AI_TOOL_AUDIT_RETENTION_DAYS` (default 90), guarded by a 24h in-memory marker.
- **Stream polish.** [`internal.ai-assistant.chat.stream.tsx`](../apps/web/app/routes/internal.ai-assistant.chat.stream.tsx) emits a `:keepalive` SSE comment every 15s while idle; empty model replies persist as `status='error'` + `error='Empty model response'` (no more `"No response generated."` placeholder); `AI_ASSISTANT_QUERY` fires on every attempt with `attempt: <n>` in metadata.
- **Import dedupe.** `applyImportSession` in [`internal.ai-assistant.tsx`](../apps/web/app/routes/internal.ai-assistant.tsx) skips messages whose `clientRequestId` already exists in the target session; the action returns `{ ok, sessionId, inserted, skipped }` so the toast can show both counts.
- **Modal proxy URL warning.** `/internal/model-setup` warns inline when `modalRemote.url` ends in `.modal.run` or matches `INTERNAL_ROUTER_UPSTREAM_URL` — operator must point assistant chat at a real chat host, not the `/route` proxy.
- **Docs propagation.** [`docs/ai-providers.md`](./ai-providers.md) gains "Release gate", "Safe target URLs", `ROUTER_REQUIRE_AUTH`, and "Auto-reprobe" subsections; [`docs/internal-admin.md`](./internal-admin.md) documents the banners, delete-vs-archive, audit retention, SSE heartbeat, empty-reply, activity-log, and import-dedupe behavior; [`README.md`](../README.md) env matrix adds `INTERNAL_AI_ALLOW_HOSTS` and `INTERNAL_AI_TOOL_AUDIT_RETENTION_DAYS`, plus the production-ignore note for `ROUTER_REQUIRE_AUTH=0`.
- **Test backlog filled.** Net +74 new vitest cases across `apps/web/app/__tests__/`: router-runtime-config (env dual-target, schema bounds, round-trip), prompt-router (dual-target parity + release-gate trip), `internal-model-setup.action.test.ts`, `assert-safe-target-url.test.ts`, `internal-assistant.service.test.ts` (fallback + both-fail), `assistant-chat-target-probe.test.ts`, `internal-ai-router.test.ts` (happy + body-max + tenant slot + suspicious prompt), `internal-assistant-store.test.ts`, `internal-ai-assistant-probe.route.test.ts`, `internal-ai-assistant-stream.test.ts` (resume / partial-persist / error / tool), `internal-ai-audit-retention.job.test.ts`, `internal-assistant-tools.test.ts`.
...
## 2026-05-01 AI provider architecture (doc)

- Documented product split in [`docs/ai-providers.md`](./ai-providers.md), [`docs/internal-admin.md`](./internal-admin.md), [`README.md`](../README.md): **module generation** = OpenAI + Claude only; **internal + first-layer** (router, assistant, model-setup) = **Qwen3 ~4B**.
...
| Ollama liveness + local defaults | `apps/web/app/services/ai/assistant-chat-target-probe.server.ts` (`probeTargetLiveness`), `apps/web/app/routes/internal.ai-assistant.tsx`, `apps/web/app/routes/internal.model-setup.tsx`, `apps/web/app/schemas/router-runtime-config.server.ts` (`DEFAULT_ROUTER_RUNTIME_CONFIG` → `http://127.0.0.1:11434` + `ollama`), `app/__tests__/assistant-chat-target-probe.test.ts`, `README.md`, `docs/internal-admin.md` | Internal AI **health** row no longer requires `/healthz` for direct Ollama (uses `GET /api/tags`), fixing false “unhealthy” when chat was actually viable; **Setup the Model** health probe matches. Default local target is direct Ollama so the assistant works without the `:8787` reference router when Ollama is running. |
...
| ~~Shopify App Subscription creation~~ **Superseded by Shopify App Pricing** | `services/billing/plan-sync.service.ts` | `BillingService.createSubscription()` / `appSubscriptionCreate` / `BILLING_TEST_MODE` were **removed** — the app no longer charges in-app. Plans are configured per-handle in the Partner Dashboard's hosted pricing page; the merchant approves there and returns via the `/billing/callback` welcome link. `PlanSyncService.syncShop()` re-fetches the plan of record from the Partner API (never trusting the callback's `plan_handle` URL param) and upserts it onto `AppSubscription`. A cron `sweep()` reconciles stale rows on a schedule (App Pricing has no webhooks). See `docs/runbooks/app-pricing-setup.md` for the Partner Dashboard setup and `plan-sync.service.ts`'s doc comments for the sync flow. |
...
Follow-up: missing `SCOPES` in `apps/web/.env` fails SSR boot with `[env] Boot failed — invalid environment: SCOPES: Required` (`validateEnv` in `env.server.ts`). Onboarding fix: `SCOPES` added to `apps/web/.env.example` (aligned with `shopify.app.toml`), with notes under Ports in `docs/internal-admin.md`.

### Verified commands (web)

- `pnpm --filter web typecheck`
- `pnpm --filter web lint` (warnings only; no blocking errors)
- `pnpm --filter web test -- app/__tests__/internal-assistant.service.test.ts app/__tests__/internal-assistant-tools.test.ts app/__tests__/router-runtime-config-schema.test.ts app/__tests__/design-reference.test.ts app/__tests__/internal-ai-router.test.ts app/__tests__/prompt-router.test.ts app/__tests__/preview-service.test.ts app/__tests__/internal-admin-route-closure.test.ts app/__tests__/assistant-chat-target-probe.test.ts`
- `pnpm --filter web build`
- `pnpm --filter web exec prisma validate`

### Authenticated depth sweep (`/internal/model-setup`, `/internal/ai-assistant`)

Session **2026-05-01** (internal dev on port 4000): after login, **Local AI Setting** (`/internal/model-setup`) showed full sections (design reference, dual-target controls, localMachine vs modalRemote URLs, operator probes, gates). **Health check target** (modalRemote) reported `/healthz` **200**. **Route contract check** reported `/route` schema **passed** (`internal_router_ok`). A transient toast overlay blocked one automated click until the Operator checks row was scrolled into view (then route check succeeded).

On **AI Assistant** (`/internal/ai-assistant`), sidebar + composer + Local / Cloud / Memory controls rendered; sending while **Cloud** was selected surfaced the expected error that modalRemote is **router-only** (needs `/v1/chat/completions`-compatible inference, not `/route` alone). After switching to **Local**, send surfaced **local target health failure** (`fetch failed`)—consistent with no local inference listening at the configured URL (`127.0.0.1:8787` in this environment). No console-critical regressions observed beyond those intentional failure modes.
