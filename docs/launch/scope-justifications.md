...
sites. Verified against `shopify.app.production.toml:126` on this branch
(master @ c201150) — the `scopes` line declares exactly 21 scopes, plus one
`optional_scopes` entry (`write_themes`, `shopify.app.production.toml:134`).
...
| `write_checkouts` | Checkout write | **No call site found.** Grepped `apps/web/app`, `packages/core`, `extensions` for `write_checkouts`, `checkoutCreate`, `checkoutUpdate`, `checkoutBranding*` — zero matches outside the toml files themselves and `docs/shopify-dev-setup.md:87` (a scope list, not a call site). The checkout UI extensions (`extensions/checkout-ui/shopify.extension.toml`) use the modern Checkout UI Extensions targets (`purchase.checkout.*`/`purchase.thank-you.*`), which do not require this scope. **Action needed before submission:** either find the real justification (re-run the grep below — a future workstream may add one) or drop `write_checkouts` from `scopes` in `shopify.app.production.toml` and `shopify.app.toml`. Do not submit with an unjustified scope — reviewers ask about exactly this. |
