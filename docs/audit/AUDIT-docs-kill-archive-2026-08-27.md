...
2. ~~`docs/gitbook/02-architecture/platform-v2-migration-plan.md` was not archived~~ — **RESOLVED in the fix-round-1 follow-up commit** (`docs(ws-j): fix round 1`). The controller ruled it in scope per the plan's "Verified ground truth" section (line 40, grouped with ADR-001/ADR-002/env-matrix/platform-hosting as retired-architecture docs); it was `git mv`'d into `docs/archive/gitbook-v2-migration/` and every inbound link fixed repo-wide (root `SUMMARY.md`, `docs/gitbook/SUMMARY.md`, `docs/archive/phase-plan.md`, `docs/archive/spec-kit-status-report.md`, the ~20 sibling files inside `docs/archive/gitbook-v2-migration/` whose `../platform-v2-migration-plan.md` relative links needed flattening to `./platform-v2-migration-plan.md`, and the outer duplicate copies still in `docs/gitbook/02-architecture/`). `specs/**` and `.specify/**` references were left untouched — out of WS-J's file-touch scope, same as Task 1.
